...
class SessionsController extends AppController
{
	...
	public function upload() 
	{
		...
		$this->loadModel('Fiches');
		$response = ['error' => false, 'message' => '', 'errors' => []];
		$fiches = $this->request->data('fiches');

        //$fiches = file_get_contents('test_data.json');
		...
		foreach ($fiches as $fiche) {
			...
			if($storedFiche) {
                ...
                $storedFiche->observations = $fiche['observations'];
				$champFiches = array_map(function($champFiche) {
					$champFiche['created'] = date('Y-m-d H:i:s');
					unset($champFiche['champ']);
                    unset($champFiche['id']);
                    $champFiche['commentaire'] = '';
					return $champFiche;
				}, $fiche['champFiches']);
               
                foreach ($champFiches as $champ) {
                    $champEntity = $this->ChampsFiches->newEntity($champ);
                    if(!$this->ChampsFiches->save($champEntity)) {
                        $response['error'] = true;
                        $response['errors'][] = $champEntity;
                    }
                }
				...
				$response['error'] = true;
				$response['message'] = 'Impossible de trouver la fiche '. $fiche['id'];
			}
		}
		$this->set(compact('response'));
	}
