...
<div class="well">
    <h3 class="page-header">Modification de Slide</h3>

    <form method="post" enctype="multipart/form-data" action="<?= $this->Url->build(['controller'=>'Slides','action'=>'edit',$slide->id])  ?>">
        ...
        <textarea name="body" id="corps" cols="30"  rows="10" >  <?= $slide->resume ?></textarea>
        <hr/>
        <label for="body">Image de l'article : </label>
        <input type="file" name="image" class="form-control" id="image" value="<?= $slide->image ?>"/>
        <hr/>
